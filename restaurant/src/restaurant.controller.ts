/* @file restaurant.controller.ts — HTTP interface layer of the Restaurant service.
 * Standard HTTP layer: validate input -> delegate to service -> JSON response.
 * @see restaurant.service.ts for business logic.
 */
...
class RestaurantController {
  // @route GET / — list all (filters + pagination via query params)
  getAllRestaurants = catchAsync(async (req, res, next) => {
    // @note req.query holds STRINGS; the schema coerces them to numbers with
    //   z.coerce (e.g. "rating=4" -> 4).
  ...
  // @route GET /:id — single restaurant
  ...
  // @route GET /:id/menu — menu items, optionally ?category=...
  ...
  // @route POST /:id/menu — add a menu item (role-protected, see routes)
  ...
  // @route POST / — create a restaurant (role-protected: admin / restaurant_owner)
  createRestaurant = catchAsync(async (req, res, next) => {
    // ! ownerId comes from the DECODED JWT on req.user (set by authenticate),
    //   never from the request body — a user cannot create a restaurant as
    //   someone else.
